...
load_environment() {
  ...
  # Determine which env file to load based on environment parameter
  case "${environment}" in
    "local")
  ...
  if [ ! -f "${env_file}" ]; then
    ...
    log_error "Expected .env.local for local or .env.prod for production"
    return 1
  fi
