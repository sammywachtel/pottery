...
class PhotoBase(BaseModel):
    ...
    fileName: Optional[str] = Field(None, description="Original filename, useful for display")
    uploadedTimezone: Optional[str] = Field(None, description="Timezone identifier (e.g., 'UTC', 'America/New_York') when photo was uploaded")
...
class Photo(PhotoBase):
    ...
    """
    id: str = Field(default_factory=lambda: str(uuid_pkg.uuid4()), description="Unique ID for the photo")
    gcsPath: str = Field(..., description="Path to the photo object in GCS (e.g., items/item_id/photo_id.jpg)")
    uploadedAt: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Timestamp (UTC) when the photo was uploaded")
...
class PhotoUpdate(BaseModel):
    """Schema for updating photo metadata fields (stage, imageNote). Timezone is not updatable here."""
...
class PotteryItemBase(BaseModel):
    ...
    createdDateTime: datetime = Field(..., description="Timestamp item was created/started (ISO 8601 format with timezone expected)")
    createdTimezone: Optional[str] = Field(None, description="Timezone identifier (e.g., 'America/New_York') when item was created")
    measurements: Optional[Measurements] = None

class PotteryItemCreate(PotteryItemBase):
    """Schema for creating a new pottery item."""
    createdTimezone: Optional[str] = Field(None, exclude=True) # Exclude from input schema if derived
