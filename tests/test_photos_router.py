...
from unittest.mock import AsyncMock, MagicMock, patch, ANY # Use ANY for variable args like photo_id
...
from models import PotteryItem, Photo, PhotoResponse, PhotoUpdate
...
async def test_upload_photo_success(client: TestClient, mocker):
    ...
    # 2. Mock GCS upload
    mock_gcs_upload = mocker.patch('services.gcs_service.upload_photo_to_gcs', new_callable=AsyncMock)
    ...
    mock_fs_add_photo = mocker.patch('services.firestore_service.add_photo_to_item', new_callable=AsyncMock)
    ...
    mock_generate_url.return_value = f"https://fake-signed-url.com/{TEST_PHOTO_ID_NEW}.jpg"
...
async def test_delete_photo_photo_not_found_in_item(client: TestClient, mocker):
    ...
    item_without_target_photo.photos = [Photo(id='other_photo', gcsPath='other/path', stage='Glazed', uploadedAt=NOW_UTC, uploadedTimezone='UTC')]
    mock_get_item_svc = mocker.patch('services.firestore_service.get_item_by_id', new_callable=AsyncMock)
...
async def test_update_photo_details_success(client: TestClient, mocker, common_headers):
    ...
    updated_photo_internal = existing_photo_internal.model_copy(update=photo_update_payload)
