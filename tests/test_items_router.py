...
@pytest.mark.asyncio
async def test_get_items_empty(client: TestClient, mocker):
...
@pytest.mark.asyncio
async def test_get_items_success(client: TestClient, mocker):
    mock_get_all = mocker.patch('services.firestore_service.get_all_items', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_get_items_firestore_error(client: TestClient, mocker):
    mock_get_all = mocker.patch('services.firestore_service.get_all_items', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_create_item_success(client: TestClient, mocker, common_headers):
    ...
    response = client.post("/api/items", json=create_payload, headers=common_headers)
...
@pytest.mark.asyncio
async def test_create_item_validation_error(client: TestClient, common_headers):
    invalid_payload = create_payload.copy()
    del invalid_payload["name"]
    response = client.post("/api/items", json=invalid_payload, headers=common_headers)
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

@pytest.mark.asyncio
async def test_create_item_validation_error_bad_date(client: TestClient, common_headers):
    invalid_payload = create_payload.copy()
...
@pytest.mark.asyncio
async def test_create_item_firestore_error(client: TestClient, mocker, common_headers):
    mock_create = mocker.patch('services.firestore_service.create_item', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_get_item_success(client: TestClient, mocker):
    ...
    response = client.get(f"/api/items/{TEST_ITEM_ID_1}")
    assert response.status_code == status.HTTP_200_OK
    json_response = response.json()
...
@pytest.mark.asyncio
async def test_get_item_not_found(client: TestClient, mocker):
    mock_get_item = mocker.patch('services.firestore_service.get_item_by_id', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_get_item_firestore_error(client: TestClient, mocker):
...
@pytest.mark.asyncio
async def test_get_item_firestore_connection_error(client: TestClient, mocker):
...
@pytest.mark.asyncio
async def test_update_item_success(client: TestClient, mocker, common_headers):
    ...
    mock_generate_urls = mocker.patch('services.gcs_service.generate_signed_urls_for_photos', new_callable=AsyncMock)
    mock_generate_urls.return_value = []
    response = client.put(f"/api/items/{TEST_ITEM_ID_1}", json=update_payload, headers=common_headers)
    assert response.status_code == status.HTTP_200_OK
    json_response = response.json()
    assert json_response["id"] == TEST_ITEM_ID_1
    response_dt_str = json_response["createdDateTime"]
...
@pytest.mark.asyncio
async def test_update_item_not_found(client: TestClient, mocker, common_headers):
    mock_update = mocker.patch('services.firestore_service.update_item_metadata', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_update_item_validation_error(client: TestClient, common_headers):
    invalid_payload = update_payload.copy()
    invalid_payload["createdDateTime"] = "invalid-date"
    response = client.put(f"/api/items/{TEST_ITEM_ID_1}", json=invalid_payload, headers=common_headers)
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

@pytest.mark.asyncio
async def test_update_item_firestore_error(client: TestClient, mocker, common_headers):
    mock_update = mocker.patch('services.firestore_service.update_item_metadata', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_delete_item_success_with_photos(client: TestClient, mocker):
    mock_get_item = mocker.patch('services.firestore_service.get_item_by_id', new_callable=AsyncMock)
    mock_get_item.return_value = sample_item_internal_1
    ...
    response = client.delete(f"/api/items/{TEST_ITEM_ID_1}")
...
@pytest.mark.asyncio
async def test_delete_item_success_no_photos(client: TestClient, mocker):
    mock_get_item = mocker.patch('services.firestore_service.get_item_by_id', new_callable=AsyncMock)
    mock_get_item.return_value = sample_item_internal_2
    mock_gcs_delete = mocker.patch('services.gcs_service.delete_multiple_photos_from_gcs', new_callable=AsyncMock)
    mock_fs_delete = mocker.patch('services.firestore_service.delete_item_and_photos', new_callable=AsyncMock)
    mock_fs_delete.return_value = True
    response = client.delete(f"/api/items/{TEST_ITEM_ID_2}")
    assert response.status_code == status.HTTP_204_NO_CONTENT
...
@pytest.mark.asyncio
async def test_delete_item_not_found(client: TestClient, mocker):
    mock_get_item = mocker.patch('services.firestore_service.get_item_by_id', new_callable=AsyncMock)
    mock_get_item.return_value = None
    mock_gcs_delete = mocker.patch('services.gcs_service.delete_multiple_photos_from_gcs', new_callable=AsyncMock)
    mock_fs_delete = mocker.patch('services.firestore_service.delete_item_and_photos', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_delete_item_gcs_error(client: TestClient, mocker):
...
@pytest.mark.asyncio
async def test_delete_item_firestore_error(client: TestClient, mocker):
