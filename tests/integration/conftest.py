...
#     pytest.exit("ERROR: Integration tests configured against non-test environment!", 1)
...
async def resource_manager():
    ...
    try:
        ...
        fs_db = firestore.Client(project=settings.gcp_project_id, database=settings.firestore_database_id)
        gcs_client = storage.Client(project=settings.gcp_project_id)
        gcs_bucket = gcs_client.bucket(settings.gcs_bucket_name)
        logger.info(f"Cleanup: Using Project='{settings.gcp_project_id}', DB='{settings.firestore_database_id}', Bucket='{settings.gcs_bucket_name}'")
    ...
    for path in created_gcs_paths:
        ...
        # *** UPDATED: Use correct NotFound exception ***
            ...
            logger.warning(f"GCS object not found during cleanup (already deleted?): {path}")
    ...
    for item_id in created_item_ids:
        if not item_id: continue # Skip empty IDs
        try:
            ...
            logger.error(f"ERROR deleting Firestore document {item_id}: {e_fs}", exc_info=True)
    created_item_ids -= deleted_ids # Remove processed IDs

    # Log any remaining resources that failed cleanup
    if created_item_ids: logger.warning(f"WARNING: Failed to clean up Firestore docs: {created_item_ids}")
    if created_gcs_paths: logger.warning(f"WARNING: Failed to clean up GCS paths: {created_gcs_paths}")
