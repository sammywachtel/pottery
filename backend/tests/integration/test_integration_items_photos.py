...
@pytest.mark.asyncio
async def test_item_create_get_delete_cycle(
    client: TestClient, resource_manager, auth_headers
):
...
async def test_item_update_cycle(client: TestClient, resource_manager, auth_headers):
    ...
    response_create = client.post(
        "/api/items", json=create_payload_data, headers=headers
    )
    assert response_create.status_code == status.HTTP_201_CREATED
    ...
    response_update = client.put(
        f"/api/items/{item_id}", json=update_payload_data, headers=headers
    )
    ...
    assert (
        update_response_data["createdDateTime"]
        == update_payload_data["createdDateTime"]
    )
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_photo_upload_get_delete_cycle(
    client: TestClient, resource_manager, auth_headers
):
    ...
    # 4. Delete Photo
    response_delete_photo = client.delete(
        f"/api/items/{item_id}/photos/{photo_id}", headers=auth_headers
    )
    assert response_delete_photo.status_code == status.HTTP_204_NO_CONTENT
    if gcs_path in created_gcs_paths:
        created_gcs_paths.remove(
            gcs_path
        )  # Remove from cleanup list as we explicitly delete
