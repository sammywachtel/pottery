...
async def _get_item_or_404(item_id: str, user_id: str) -> PotteryItem:
    ...
    try:
        ...
        if not item:
            logger.warning(
                f"Item not found or not owned by user {user_id} for ID: {item_id}"
            )
            raise HTTPException(
        ...
        logger.error(
            f"Service connection error fetching item {item_id}: {conn_err}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Backend database service unavailable.",
        )
...
async def _create_item_response(item: PotteryItem) -> PotteryItemResponse:
    ...
    item_response_data["photos"] = (
        photo_responses  # Replace internal photos with response photos
    )
...
@router.get(
    ...
    description=(
        "Retrieves a list of pottery items belonging to the authenticated user, "
        "including temporary signed URLs for photos."
    ),
    responses={
...
async def get_items(current_user: User = Depends(get_current_active_user)):
    """
    Retrieves pottery items belonging to the authenticated user from Firestore
    and generates signed URLs for their photos.
    """
    try:
...
@router.post(
    ...
    description=(
        "Creates a new pottery item metadata entry in Firestore associated with "
        "the authenticated user. Photos should be uploaded separately. Returns "
        "the created item (photos list will be empty)."
    ),
    responses={
...
async def create_item(
    item_create: PotteryItemCreate,
    current_user: User = Depends(get_current_active_user),
):
    """
    Creates a new pottery item document in Firestore associated with
    the authenticated user.
    """
    try:
        # Associate the item with the current user
        new_item = await firestore_service.create_item(
            item_create, user_id=current_user.username
        )
        # The new item initially has no photos, so the response conversion is simple
        response_item = await _create_item_response(
            new_item
        )  # Will have empty photo list
...
@router.get(
    ...
    description=(
        "Retrieves a single pottery item by its ID belonging to the "
        "authenticated user, including temporary signed URLs for photos."
    ),
    responses={
        status.HTTP_200_OK: {"description": "Successful Response"},
        status.HTTP_401_UNAUTHORIZED: {
...
async def get_item(item_id: str, current_user: User = Depends(get_current_active_user)):
    """
    Retrieves a specific item by ID belonging to the authenticated user
    and generates signed URLs for its photos.
    """
    ...
    try:
        ...
        logger.error(
            f"Service connection error generating response for {item.id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
        ...
        logger.error(
            f"Error generating response for item {item.id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to process item data.",
        )
...
@router.put(
    ...
    description=(
        "Updates an existing pottery item's metadata (name, clay, notes, etc.) "
        "in Firestore belonging to the authenticated user. Does not handle "
        "photo list updates directly. Returns the updated item including photo "
        "metadata with fresh signed URLs."
    ),
    responses={
        status.HTTP_200_OK: {"description": "Successful Response"},
        status.HTTP_401_UNAUTHORIZED: {
...
async def update_item(
    ...
    try:
        ...
        updated_item = await firestore_service.update_item_metadata(
            item_id, item_update, user_id=current_user.username
        )
        if updated_item is None:
            # This case happens if the item disappeared between check and update,
            # if service returns None on not found, or if the item doesn't
            # belong to the user
            logger.warning(
                f"Item {item_id} not found or not owned by user "
                f"{current_user.username} during update operation."
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Pottery item with ID '{item_id}' not found.",
        ...
        logger.error(
            f"Service connection error in update_item for {item_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Backend service unavailable.",
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to update item metadata.",
        )
...
@router.delete(
    ...
    description=(
        "Deletes a pottery item belonging to the authenticated user from "
        "Firestore and its associated photos from GCS."
    ),
    responses={
        status.HTTP_204_NO_CONTENT: {
            "description": (
                "No Content - Item and associated photos successfully deleted "
                "(or item did not exist)."
            )
        },
        status.HTTP_401_UNAUTHORIZED: {
            "model": HTTPError,
...
async def delete_item(
    ...
    """
    Deletes an item belonging to the authenticated user and all its
    associated photos.
    """
    ...
    # Handles 404/500/503 for initial fetch
    item = await _get_item_or_404(item_id, user_id=current_user.username)
    ...
    if gcs_paths_to_delete:
        try:
            gcs_delete_success = await gcs_service.delete_multiple_photos_from_gcs(
                gcs_paths_to_delete
            )
            if not gcs_delete_success:
                logger.error(
                    f"Failed to delete one or more GCS photos for item {item_id}."
                )
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to delete associated photos from storage.",
                )
        except ConnectionError as e:
            logger.error(
                f"GCS connection error during photo deletion for item {item_id}: {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            ...
            logger.error(
                f"Unexpected error deleting GCS photos for item {item_id}: {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Error during photo deletion.",
            )

    # 3. Delete item from Firestore (pass user_id to ensure ownership check)
    try:
        firestore_delete_success = await firestore_service.delete_item_and_photos(
        ...
        if not firestore_delete_success:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Failed to delete item metadata after deleting photos.",
            )
        ...
        logger.error(
            f"Firestore connection error during item deletion for {item_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
        ...
        logger.error(
            f"Unexpected error deleting item {item_id} from Firestore: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to delete item metadata.",
        )
