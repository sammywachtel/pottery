...
async def lifespan(app: FastAPI):
    ...
    # Check if Firestore/GCS clients initialized correctly during service module import
    ...
    if not firestore_service.db or not gcs_service.bucket:
         logger.critical("FATAL: Firestore or GCS client failed to initialize during module import. Application may not function correctly.")
    ...
    # For google-cloud libraries, explicit closing is often not required as clients manage connections.
...
# Note: You could potentially add logic here to dynamically determine the Cloud Run URL
# if running in that environment, using settings.service_name etc., but the placeholder is simpler.
...
async def google_cloud_exception_handler(request: Request, exc: GoogleCloudError):
    ...
    # Determine appropriate status code (e.g., 503 for connectivity, 500 for others)
    status_code = status.HTTP_503_SERVICE_UNAVAILABLE if "unavailable" in str(exc).lower() else status.HTTP_500_INTERNAL_SERVER_ERROR
    return JSONResponse(
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Uvicorn server locally on host 0.0.0.0, port {settings.port}...")
    uvicorn.run(
